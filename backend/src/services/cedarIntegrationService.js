...
class CedarIntegrationService {
    ...
    async initializePool() {
        try {
            ...
            console.error('❌ Cedar Integration Service: Database connection failed:', error.message);
        }
    }
    ...
    async createWorkOrder(ticketData) {
        ...
        try {
            ...
            const woData = this.prepareWorkOrderData(ticketData, currentDate, currentTime);
            ...
            await this.updateTicketWithCedarInfo(ticketData.ticketId, woNo, woCode, 'success');

            // Log successful integration
            await this.logIntegration(ticketData.ticketId, woNo, 'create', 'success', woData, {
                wono: woNo,
                wocode: woCode
            });
            ...
            await this.updateTicketWithCedarInfo(ticketData.ticketId, null, null, 'error', error.message);

            // Log failed integration
            await this.logIntegration(ticketData.ticketId, null, 'create', 'error', ticketData, null, error.message);
    ...
    async updateWorkOrderStatus(ticketId, ticketStatus, updateData = {}) {
        try {
            ...
            if (!ticket.cedar_wono) {
                console.log(`⚠️ Ticket ${ticketId} has no Cedar WO - skipping status update`);
                return { success: false, message: 'No Cedar WO found for this ticket' };
            }

            console.log(`🔄 Updating Cedar WO ${ticket.cedar_wono} status to ${ticketStatus}`);

            // Get status mapping
            const statusMapping = this.getStatusMapping(ticketStatus);
            if (!statusMapping) {
                console.log(`⚠️ No status mapping found for ${ticketStatus} - skipping update`);
                return { success: false, message: `No status mapping for ${ticketStatus}` };
            }

            // Prepare update data
            const updateFields = this.prepareStatusUpdateFields(ticketStatus, updateData, ticket);
            ...
            const updateQuery = `UPDATE WO SET ${updateFields.map(f => `${f.field} = @${f.name}`).join(', ')} WHERE WONO = @wono`;
            await updateRequest.query(updateQuery);

            // Update ticket sync status
            await this.updateTicketWithCedarInfo(ticketId, ticket.cedar_wono, ticket.cedar_wocode, 'success');

            // Log successful integration
            await this.logIntegration(ticketId, ticket.cedar_wono, 'status_update', 'success', {
                ticketStatus,
                statusMapping,
                updateData
            }, { updated: true });

            console.log(`✅ Cedar WO ${ticket.cedar_wono} status updated to ${ticketStatus}`);
            ...
            // Update ticket with error information
            await this.updateTicketWithCedarInfo(ticketId, null, null, 'error', error.message);

            // Log failed integration
            await this.logIntegration(ticketId, null, 'status_update', 'error', {
                ticketStatus,
                updateData
            }, null, error.message);
    ...
    async syncTicketToCedar(ticketId, action, actionData = {}) {
        try {
            ...
            const ticketResult = await this.pool.request()
                .input('ticketId', sql.Int, ticketId)
                .query(`
                    SELECT t.*, pu.PUCODE, pu.PUNAME, pu.COSTCENTERNO, pu.DEPTNO
                    FROM Tickets t
            ...
            if (action === 'accept' && !ticket.cedar_wono) {
                console.log(`🔄 Creating Cedar WO for ticket ${ticketId} (action: ${action})`);
                ...
                console.log(`🔄 Updating Cedar WO ${ticket.cedar_wono} (action: ${action})`);
                return await this.updateWorkOrderStatus(ticketId, ticket.status, actionData);
            } else {
                // No WO exists and action is not 'accept' - this is normal for open tickets
                console.log(`ℹ️ No Cedar WO exists for ticket ${ticketId} (action: ${action}) - this is normal for open tickets`);
                return {
                    success: true,
                    message: 'No Cedar WO exists - WO will be created when ticket is accepted',
                    wono: null,
                    wocode: null
                };
            }
            ...
            console.error(`❌ Failed to sync ticket ${ticketId} to Cedar:`, error.message);
            throw error;
        }
    }
    ...
    prepareWorkOrderData(ticketData, currentDate, currentTime) {
        ...
        return {
            ...
            woPlan: '',
            woCause: '',
            deptNo: ticketData.deptNo, // Default department
            ...
            requesterName: ticketData.requestedBy || 'System User',
    ...
    setWorkOrderParameters(request, woData) {
        ...
        request.input('DEPT_REQ', sql.Int, woData.deptNo);
    ...
    prepareStatusUpdateFields(ticketStatus, updateData, ticket) {
        ...
        const updateFields = [
            { name: 'updateUser', type: sql.Int, field: 'UPDATEUSER', value: updateData.changedBy || 1 },
            { name: 'updateDate', type: sql.NVarChar, field: 'UPDATEDATE', value: currentDate },
            { name: 'updateTime', type: sql.NVarChar, field: 'UpdateTime', value: currentTime }
        ];

        // Add status-specific fields
        if (statusMapping) {
            updateFields.push(
                { name: 'woStatusNo', type: sql.Int, field: 'WOStatusNo', value: statusMapping.woStatusNo },
                { name: 'wfStatusCode', type: sql.NVarChar, field: 'WFStatusCode', value: statusMapping.wfStatusCode }
            );
        }
        ...
        switch (ticketStatus) {
            case 'planed':
                ...
                const scheduleStart = updateData.scheduleStart || updateData.schedule_start || ticket.schedule_start;
                const scheduleFinish = updateData.scheduleFinish || updateData.schedule_finish || ticket.schedule_finish;
                const assignedTo = updateData.assignedTo || updateData.assigned_to || ticket.assigned_to;
                ...
                if (scheduleStart && scheduleFinish && assignedTo) {
                    ...
                    updateFields.push(
                        { name: 'schStartDate', type: sql.NVarChar, field: 'SCH_START_D', value: startDateStr },
                        { name: 'schStartTime', type: sql.NVarChar, field: 'SCH_START_T', value: startTimeStr },
                        { name: 'schFinishDate', type: sql.NVarChar, field: 'SCH_FINISH_D', value: finishDateStr },
                        { name: 'schFinishTime', type: sql.NVarChar, field: 'SCH_FINISH_T', value: finishTimeStr },
                        { name: 'assignTo', type: sql.Int, field: 'ASSIGN', value: assignedTo },
                        { name: 'repairBy', type: sql.Int, field: 'REPAIRBY', value: assignedTo },
                        { name: 'workBy', type: sql.Int, field: 'WORKBY', value: assignedTo }
                    );
                }
                break;
            ...
            case 'in_progress':
                ...
                const actualStartAt = updateData.actualStartAt || ticket.actual_start_at;
                if (actualStartAt) {
                    ...
                    updateFields.push(
                        { name: 'actStartDate', type: sql.NVarChar, field: 'ACT_START_D', value: startDateStr },
                        { name: 'actStartTime', type: sql.NVarChar, field: 'ACT_START_T', value: startTimeStr },
                        { name: 'workBy', type: sql.Int, field: 'WORKBY', value: ticket.assigned_to || updateData.changedBy }
                    );
                }
                break;
            ...
            case 'finished':
                ...
                const actualFinishAt = updateData.actualFinishAt || ticket.actual_finish_at || ticket.finished_at;
                if (actualFinishAt) {
                ...
                const actualStartAtFinish = updateData.actualStartAt || ticket.actual_start_at;
            ...
            case 'closed':
                updateFields.push(
                    ...
                    { name: 'acceptDate', type: sql.NVarChar, field: 'ACCEPTDATE', value: currentDate },
                    { name: 'acceptTime', type: sql.NVarChar, field: 'ACCEPT_TIME', value: currentTime },
                    { name: 'acceptNote', type: sql.NVarChar, field: 'ACCEPT_NOTE', value: updateData.notes },
                    { name: 'acceptUser', type: sql.Int, field: 'ACCEPTUSER', value: updateData.changedBy },
                    { name: 'hisDate', type: sql.NVarChar, field: 'HIS_DATE', value: currentDate },
                    { name: 'hisTime', type: sql.NVarChar, field: 'HIS_TIME', value: currentTime }
                );
                break;
        }
     ...
     */
    async updateTicketWithCedarInfo(ticketId, wono, wocode, syncStatus, errorMessage = null) {
        try {
            const updateRequest = this.pool.request()
                .input('ticketId', sql.Int, ticketId)
     ...
     */
    async logIntegration(ticketId, wono, action, status, requestData, responseData, errorMessage = null) {
        try {
            const logRequest = this.pool.request()
                .input('ticketId', sql.Int, ticketId)
                .input('wono', sql.Int, wono)
                .input('action', sql.VarChar(50), action)
                .input('status', sql.VarChar(20), status)
                .input('requestData', sql.NVarChar(sql.MAX), JSON.stringify(requestData))
                .input('responseData', sql.NVarChar(sql.MAX), responseData ? JSON.stringify(responseData) : null)
